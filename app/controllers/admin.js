...
module.exports = {
 ...
 deleteProduct: function(req, res) {
  ...
  //  kit.models.homeBox.delete({ contentId: id, type: 'brands' }, function() {
      kit.models.product.delete({ id: id }, function() {
        res.redirect('/admin/products');
      });
   // });
  },
  ...
  deleteProductPicture: function(req, res) {
    ...
    kit.models.product.update({ id: id }, { picture: '' } , function(err) {
    ...
    kit.models.product.find({ where: { id: productId } }, function(err, results) {
     //   console.log(results[0]);
      ...
      kit.models.image.find({where:{ entity: productId, type:"product" }, order: ['image.order ASC'] }, function(err, images){
          ...
          res.render('admin/editProduct', { product: results[0], images: images, moment: moment, errors: req.flash('productError') || [] });
      });
    });
  },
...
if(_.isArray(order)){
    ...
    kit.models.product.update({ id: id }, product, function(err) {
      if (err) {
        req.flash('productError', err.toString());
        res.redirect(req.originalUrl);
        return
      }
      res.redirect('/admin/products');
    ...
    usedsCreateView: function(req, res) {
        var form = req.flash('form');
     //   console.log(form,"usedsCreateView","Arman");
        res.render('admin/createUsed', { errors: req.flash('usedError') || [] ,
            form: form.length > 0 ? form[0] : {}});
    },

    usedsCreate: function(req, res) {
        var model = locales.encode(req.body.model)
            , description = locales.encode(req.body.description)
            , parameters = locales.encode(req.body.parameters)
            , type = req.body.type;
      //  console.log(req.body,"usedsCreate","Arman");



        var used = kit.models.used.build({
            model: model,
            description: description,
            parameters: parameters,
            type: type
        ...
        if (req.files.picture && req.files.picture.size > 0) {
            if (config.get('allowedImageTypes').indexOf(req.files.picture.type) != -1) {
                ...
                var os = fs.createWriteStream(config.get('path').uploads + "/" + newFileName);
                is.pipe(os);
                used.picture = newFileName;
            }
        }

        used.save(function(err, id) {
            if (err) {
                req.flash('brandError', err.toString());
                req.flash('form', req.body);
                res.redirect(req.originalUrl);
            ...
            res.redirect('/admin/useds');
        });
    },

    deleteUsed: function(req, res) {
        var id = parseInt(req.query.id);
        ...
        //  kit.models.homeBox.delete({ contentId: id, type: 'brands' }, function() {
        kit.models.used.delete({ id: id }, function() {
            res.redirect('/admin/useds');
        });
        // });
    },

    deleteUsedPicture: function(req, res) {
        var id = parseInt(req.param('id'));
        ...
        kit.models.used.update({ id: id }, { picture: '' } , function(err) {
            res.redirect('/admin/used/' + id);
        });
    },
    deleteUsedImages: function(req, res) {
        var id   = parseInt(req.param('id'));

        if (isNaN(id)) {
            res.redirect('/admin/used');
            return;
        }
    ...
    used: function(req, res) {
        ...
        kit.models.used.find({ where: { id: usedId } }, function(err, results) {
            ...
            kit.models.image.find({where:{ entity: usedId, type:"used" }, order: ['image.order ASC'] }, function(err, images){
                ...
                res.render('admin/editUsed', { used: results[0], images: images, moment: moment, errors: req.flash('usedError') || [] });
            });
        });
    },

    updateUsed: function(req, res) {
        var model = locales.encode(req.body.model)
            , description = locales.encode(req.body.description)
            , parameters = locales.encode(req.body.parameters)
            , type = req.body.type
        ...
        kit.models.used.update({ id: id }, used, function(err) {
            if (err) {
                req.flash('productError', err.toString());
                res.redirect(req.originalUrl);
                return
